...
<div class="row">
				...
				<div class="col-md-6">
					<div class="form-group">
						...
						<select class="form-control select-two" name="cbStatus" data-placeholder=" -- Pilih Status -- " required="required">
							<option></option>
							<option value="<?=STATUS_ENABLE;?>" <?php echo set_select(STATUS_ENABLE, $arr_participant_group['participant_group_status']); ?>>Enable</option>
							<option value="<?=STATUS_DISABLE;?>" <?php echo set_select(STATUS_DISABLE, $arr_participant_group['participant_group_status']); ?>>Disable</option>
						</select>
						<ul class="fields-message"></ul>
					</div>
				</div>
	...
	<div class="col-md-5 col-sm-12 mg-t-20 mg-md-t-0">
		...
		<div class="card shadow-base bd-0 mg-b-20">
			<?php if (!empty($arr_participant_group['updated']) && !empty($arr_participant_group['updated_by']) && !empty($arr_participant_group['created']) && !empty($arr_participant_group['created_by'])): ?>
			<div class="card-body bg-transparent pd-0 bd-gray-200 mg-t-auto">
				<div class="row no-gutters tx-center">
					<?php if (!empty($arr_participant_group['updated']) && !empty($arr_participant_group['updated_by'])): ?>
					<div class="col pd-y-15">
						...
						<h4 class="tx-16 tx-bold mg-b-0 tx-inverse">
							<?=strtoupper($arr_participant_group['updated_by']);?>
						</h4>
						<span class="tx-12 tx-primary tx-roboto">
							<?=timestamp_to_date($arr_participant_group['updated']);?>
						</span>
					</div>
					<?php endif;?>
					...
					<div class="col pd-y-15 bd-l bd-gray-200">
						<?php if (!empty($arr_participant_group['created']) && !empty($arr_participant_group['created_by'])): ?>
						<p class="mg-b-5 tx-uppercase tx-12 tx-mont tx-semibold">Dibuat</p>
						<h4 class="tx-16 tx-inverse tx-bold mg-b-0">
							<?=strtoupper($arr_participant_group['created_by']);?>
						</h4>
						<span class="tx-12 tx-primary tx-roboto">
							<?=timestamp_to_date($arr_participant_group['created']);?>
						</span>
					</div>
					<?php endif;?>
				</div>
