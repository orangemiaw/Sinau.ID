...
if($action == 'answer') {
    if(empty($answer) || $answer == 'undefined') {
        ...
        ajax_output(array(), 400, array('location' => '?page=exam&id=' . $id*1909 . '&hal=' . $question_no));
    }
    ...
    if(!empty($json->error_message)) {
        ...
        // $arr_question   = $m_question->get_results(array('question_type_id' => $id), $question_no, 1);
        $arr_type       = $m_question_type->get_row(array('question_type_id' => $id));
        $total_question = $arr_type['total'];
        if($question_no < $total_question) {
            $redirection = '?page=exam&id=' . $id*1909 . '&hal=' . ($question_no+1);
        } else {
        ...
        ajax_output(array(), $response_code, array('location' => $redirection));
    } else {
