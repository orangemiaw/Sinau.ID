...
if($name)
    $where['admin_group_name'] = $name;
if($status)
    $where['admin_group_status'] = $status;

$m_admin_group   = new model_admin_group($db);
$page_number     = is_numeric($_GET['hal']) ? $_GET['hal'] : 1;
$data_per_page   = 20;
$total_rows      = $m_admin_group->total_rows();
$arr_admin_group = $m_admin_group->get_results($where, $page_number, $data_per_page);
?>
        ...
        <div class="card bd-0 shadow-base pd-15">
            ...
            <div class="bg-gray-300 bd pd-15 mg-b-15 rounded">
                <form method="GET" action="<?=HTTP;?>">
                    <input type="hidden" name="page" value="admin_group">
            ...
            <div class="card-block mg-b-15">
                ...
                <?php if (isset($_SESSION['role']->{$_GET['page']}->add)): ?>
                    <a href="<?=HTTP.'?add=admin_group';?>" class="btn btn-primary">
                ...
                <div class="bd bd-gray-300 rounded table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <?php foreach($arr_admin_group as $value): ?>
                            <tr>
                                <td class="min-w">
                                ...
                                <td class="align-middle">
                                    <strong><?=strtoupper($value['admin_group_name']);?></strong>
                                </td>
                                <td class="align-middle text-center">
                                    <?php if ($value['admin_group_status'] == STATUS_ENABLE): ?>
                                        <span class="badge badge-success">Enable</span>
                                    <?php elseif ($value['admin_group_status'] == STATUS_DISABLE): ?>
                                ...
                                <td class="min-w text-center align-middle">
                                    <?php if (isset($_SESSION['role']->{$_GET['page']}->update)): ?>
                                        <a href="<?=HTTP . '?update=admin_group&id=' . $value['admin_group_id']*1909;?>" class="btn btn-outline-primary btn-icon rounded-circle" data-toggle="tooltip" data-placement="bottom" title="Ubah">
                                            <div class="tx-20"><i class="icon ion-md-create"></i></div>
                                        </a>
                                    <?php endif;?>

                                    <?php if (isset($_SESSION['role']->{$_GET['page']}->detail)): ?>
                                        <a href="<?=HTTP . '?detail=admin_group&id=' . $value['admin_group_id']*1909;?>" class="btn btn-outline-info btn-icon rounded-circle" data-toggle="tooltip" data-placement="bottom" title="Detail">
                                            <div class="tx-20"><i class="icon ion-md-camera"></i></div>
                                        </a>
                                    <?php endif;?>
                                    
									<?php if (isset($_SESSION['role']->{$_GET['page']}->delete) && $value['admin_group_name'] != 'Super Admin'): ?>
									<a href="javascript:;" onclick="deleteConfirm('<?=HTTP . '?do=admin_group&act=delete&id=' . $value['admin_group_id']*1909;?>');" class="btn btn-outline-danger btn-icon rounded-circle" data-toggle="tooltip" data-placement="bottom" title="Delete">
