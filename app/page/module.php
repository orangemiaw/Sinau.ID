...
$module = isset($_GET['txtModule']) ? $_GET['txtModule'] : false;
$status = isset($_GET['cbStatus']) ? $_GET['cbStatus'] : false;

if($name)
    $where['module_text'] = $module;
if($status)
...
$data_per_page  = 20;
$total_rows     = $m_module->total_rows($where);
$arr_module     = $m_module->get_results($where, $page_number, $data_per_page);
?>
        ...
        <div class="card bd-0 shadow-base pd-15">
            ...
            <div class="card-block mg-b-15">
                ...
                <?php if (isset($_SESSION['role']->{$_GET['page']}->add)): ?>
                    <a href="<?=HTTP.'?add=module';?>" class="btn btn-primary">
